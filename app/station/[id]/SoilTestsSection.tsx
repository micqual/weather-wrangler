...
export default function SoilTestsSection({
  ...
  return (
    <div>
      ...
      {nitrogenTests.length === 0 && <p style={{ fontSize: 13, color: 'var(--text-muted)', margin: '8px 0 0' }}>No nitrogen tests recorded yet.</p>}
      {nitrogenTests.map(t => (
        <div key={t.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'var(--text-muted)', padding: '6px 0', borderTop: '1px solid var(--border)', marginTop: 4 }}>
          <span>
      ...
      {phosphorusTests.length === 0 && <p style={{ fontSize: 13, color: 'var(--text-muted)', margin: '8px 0 0' }}>No phosphorus tests recorded yet.</p>}
      {phosphorusTests.map(t => (
        <div key={t.id} style={{ padding: '6px 0', borderTop: '1px solid var(--border)', marginTop: 4 }}>
